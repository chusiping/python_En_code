...
def extract_number_from_brackets(text_array):
    """
    从字符串数组或字符串中提取括号内的数字
    例如：["东(103)"] -> [103] 或 ["东(103)", "西(45)"] -> [103, 45] 或 "东(103)" -> 103
    参数可以是字符串或数组
    """
    results = []
    # 确保输入是数组
    ...
    elif not isinstance(text_array, (list, tuple)):
        # 如果是其他类型，转换为列表
...
def add_random_to_sequence(data):
    result = []
    ...
    for i in range(len(data)):
        ...
        if i == 0 or data[i] != data[i-1]:
            ...
            # 当前行与前一行相等，使用相同的随机数
            random_num = result[i-1][1]  # 使用前一行的随机数
        
        result.append((data[i], random_num))
...
# 读取 Excel，校验 name 列唯一，重复抛出异常
def readExcelAndCheckUniqueNames(file_path):
    import pandas as pd
    
    df = pd.read_excel(file_path)
    
    if 'name' not in df.columns:
        raise ValueError("Excel 文件中缺少 'name' 列")
    ...
    for name in names:
        if name in seen:
            raise ValueError(f"重复的 name 值: {name}")
        seen.add(name)
...
if __name__ == "__main__":
    ...
    # 应用函数
    result = add_random_to_sequence(data)

    # 输出结果
    for value, rand_num in result:
        print(f"原值: {value}, 随机数: {rand_num}, 输出: {value}{rand_num}")
